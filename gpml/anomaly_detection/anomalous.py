"""
This module provides an implementation of ANOMALOUS.

Peng, Z., Luo, M., Li, J., Liu, H., & Zheng, Q. (2018, July).
ANOMALOUS: A Joint Modeling Approach for Anomaly Detection on Attributed Networks.
In IJCAI (Vol. 18, pp. 3513-3519).

It includes a class to process NetworkX graphs and numpy arrays for attributes
to retrieve anomalies.

Class:
    ANOMALOUS - ANOMALOUS Algorithm class

Public Methods:
    fit - feed data to the ANOMALOUS object and compute anomalies.
...
class ANOMALOUS(Detector):
    ...
    def fit(self, alpha: float, beta: float, gamma: float, phi: float, epochs: int = 100, verbose: bool = False):
        """
        ...
        :param phi: Parameter phi of the algorithm
        ...
        def loss_function():
            return tf.norm(X - (X @ W @ X) - R, ord='fro', axis=(0,1)) \
                + alpha * self.l21_norm(W) \
                + beta * self.l21_norm(tf.transpose(W)) \
                + gamma * self.l21_norm(tf.transpose(R)) \
                + phi * tf.linalg.trace(R @ L @ tf.transpose(R))
        ...
        R = tf.Variable(inv((gamma + 1) * np.eye(n) + phi * L) @ X, dtype=tf.float32)
